 ...
 */
(function (global, factory) { // 检查 CommonJS 和 require.js 
  ...
  var emptyObject = Object.freeze({}); // 定义空对象并冻结，后面方便比对

  // These helpers produce better VM code in JS engines due to their
  // explicitness and function inlining.
  function isUndef (v) { // 判断未定义 值为undefined或者null都有可能是未定义的
    return v === undefined || v === null
  }

  function isDef (v) { // 判断已定义，值不等于undefined和null的为已定义的
    return v !== undefined && v !== null
  }

  function isTrue (v) { // 判断true
    return v === true
  }

  function isFalse (v) { // 判断false
    return v === false
  }
   ...
   */
  function isPrimitive (value) { // 判断是否为原始类型
    return (
      typeof value === 'string' ||
      typeof value === 'number' ||
      // $flow-disable-line
  ...
  function toRawType (value) { // 通过切割引用类型得到后面的基本类型，比如数组就是[object Array]，然后拿到后面的Array
   ...
   */
  function isPlainObject (obj) { // 判断纯粹的对象，通过new，字面量，Object.create创建出来的
    return _toString.call(obj) === '[object Object]'
  }

  function isRegExp (v) { // 判断正则
   ...
   */
  function isValidArrayIndex (val) { // 验证是否是一个有效的索引，本质是验证是否是一个非无穷大的正整数
    var n = parseFloat(String(val)); // 先转浮点
    return n >= 0 && Math.floor(n) === n && isFinite(val) // 0大于等于0，且n向下取整等于自己，且是一个有限的数
  }

  function isPromise (val) { // 判断promise
    return (
      isDef(val) && // 已定义
      typeof val.then === 'function' && // promise可以点出来then，并且是个函数
      typeof val.catch === 'function'// promise可以点出来catch，并且是个函数
    )
  }
   ...
   */
  function toString (val) { // 转换成string
    return val == null
      ? '' // 如果是null就直接返回‘’
      : Array.isArray(val) || (isPlainObject(val) && val.toString === _toString) // 如果是数组 || 对象（复合纯粹的对象且点出来的toString和Object.prototype.toString一样的）
        ? JSON.stringify(val, null, 2) // 使用JSON.stringify()转这个值，三个参数代表 值、不过滤、美化2个缩进
        : String(val) // 不是数组或者复合条件的对象，直接String转
  }
   ...
   */
  function toNumber (val) { // 转换成Number
    var n = parseFloat(val); // 先转换成浮点
    return isNaN(n) ? val : n // 转完如果是NaN，就返回原值，不然就返回n
  }
   ...
   * is in that map.
   */
  function makeMap ( // 将字符串切割后放到map中，用于校验某个字符串是否存在与map中
    str,
    expectsLowerCase
  ) {
    var map = Object.create(null); // 创建一个空对象
    var list = str.split(',');  // 将参数str用逗号切割成数组
    for (var i = 0; i < list.length; i++) {
      map[list[i]] = true; // 往空对象中添加键值对，key为list中的每一项，值为true
    }
    return expectsLowerCase // 如果参数传过来要求返回小写，就return小写的map，不然就原值返回
   ...
   */
  var isBuiltInTag = makeMap('slot,component', true); // 判断是否为内置标签

  /**
   * Check if an attribute is a reserved attribute.
   */
  var isReservedAttribute = makeMap('key,ref,slot,slot-scope,is'); // 判断是否是保留属性

  /**
   * Remove an item from an array.
   */
  function remove (arr, item) { // 从数组中移除一个项目
    if (arr.length) { // 如果传过来的数组长度不为0
      var index = arr.indexOf(item); // 适应indexOf获得某一项在数组中的索引
      if (index > -1) { // 如果找到了
        return arr.splice(index, 1) // 从那个索引开始删除长度1
      }
    }
  }
   ...
   * 判断对象是否含有某一个属性
   */
  var hasOwnProperty = Object.prototype.hasOwnProperty; // 声明一个变量并赋值上Object.prototype上的hasOwnProperty
  function hasOwn (obj, key) {
    return hasOwnProperty.call(obj, key) // 使用对象原型上的方法，使用call将this指向到传入的obj上，然后检测key
  }
   ...
   */
  function cached (fn) {
    var cache = Object.create(null); // 创建空对象，用户缓存结果
    return (function cachedFn (str) { // 闭包 获取缓存对象str属性的值，如果该值存在，直接返回，不存在调用一次fn，然后将结果存放到缓存对象中
      var hit = cache[str];
   ...
   * 驼峰化一个连字符连接的字符串
   */
  var camelizeRE = /-(\w)/g; // 声明匹配-的正则
  var camelize = cached(function (str) { // 创建纯函数
    // replace的第二个参数可以为函数，函数的第一个参数匹配到的是 -x，第二个参数是x，将x转大写使用replace替换即可
    // 详见MDN string.prototype.replace
  ...
  function polyfillBind (fn, ctx) { // 每次加元素计算数组长度
    function boundFn (a) {
  ...
  function nativeBind (fn, ctx) { // bind更改this指向
    return fn.bind(ctx)
  }

  // 如果函数原型上有bind就使用，不然使用polyfill写法
  ...
  function toArray (list, start) {
    start = start || 0; // 默认不跳过元素
    var i = list.length - start; // 长度－ 跳过个数为最后return出去数组的长度
    var ret = new Array(i); // 生成长度正确的数组
    while (i--) { // 从最后一个元素开始赋值
      ret[i] = list[i + start]; // 每一个元素等于类数组的第i+跳过的索引
    }
    return ret
  }
  ...
  function extend (to, _from) {
    for (var key in _from) { // 使用for in 将第二个参数的每一项给第一个对象
      to[key] = _from[key]; // to的键(key) = _from对应的值(_from[key])
    }
    return to
  }
  ...
  function toObject (arr) {
    var res = {}; // 创建一个空对象，最后return这个
    for (var i = 0; i < arr.length; i++) { // 循环传过来的对象数组
      if (arr[i]) { // 如果对象存在
        extend(res, arr[i]); // 使用extend函数将该对象插入指定对象
      }
    }
    return res
  }
  ...
  function genStaticKeys (modules) {
    //对传过来的数组的每一项执行函数,reduce的第一个参数是函数，参数含义为
    // 第1个.累计器累计回调的返回值; 它是上一次调用回调时返回的累积值，或最后一行那个空数组
    // 第2个.数组中正在处理的元素
    // 详见Array.prototype.reduce MDN:https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Array/Reduce
    return modules.reduce(function (keys, m) {
      return keys.concat(m.staticKeys || []) // 连接两个数组
    }, []).join(',')// 用join将这个数组转换为用逗号分隔的字符串
  }
  ...
  /**
    ...
    * 判断两个对象键名与键值对应相同 而非指引用地址相同
    * ES6中新方法Object.is(a,b) 实现两个集合深比较，会比较地址
    */
  function looseEqual (a, b) {
    if (a === b) { return true } // 参数a全等b直接返回true
    var isObjectA = isObject(a);
    var isObjectB = isObject(b);
    if (isObjectA && isObjectB) { // 如果a,b不为空，且都是对象
      try {
        var isArrayA = Array.isArray(a);
        var isArrayB = Array.isArray(b);
        if (isArrayA && isArrayB) { // 因为数组也是对象，再判断是否都是数组
          // 首先判断数组长度是否相等，不相同则结束
        ...
        } else if (!isArrayA && !isArrayB) { // 如果a，b都是对象
          var keysA = Object.keys(a); // keysA为对象a键名组成的数组
          var keysB = Object.keys(b); // keysB为对象b键名组成的数组
          // 判断两个键名组成的数组长度是否相等，不相同则结束
    ...
    } else if (!isObjectA && !isObjectB) { // 不是对象也不是数组，转字符串比较
   ...
   * 返回一个元素在数组中的索引，-1表示没找到，找到了就调用上面的浅比较
   */
   ...
   * 确保一个函数只运行一次
   * 这里使用了闭包，函数返回值是一个函数，第一次调用后called被设置成true，因为闭包，再调用时这个called为true，所以不会调用第二次
   */
  function once (fn) {
    var called = false;
    return function () {
  ...
  **/
  ...
  var componentVNodeHooks = {  //组件钩子函数
    init: function init (vnode, hydrating) { //初始化组件函数
      if (
    ...
    insert: function insert (vnode) { //安装插入
      ...
      if (!componentInstance._isMounted) {
        ...
        callHook(componentInstance, 'mounted'); //触发mounted钩子函数
      }
  ...
  function createComponent (
    ...
    if (isUndef(Ctor.cid)) { //组件的id 唯一标识符
      asyncFactory = Ctor;
      // 解决异步组件 更新组建数据
      Ctor = resolveAsyncComponent(asyncFactory, baseCtor); //返回组件现在的状态
      if (Ctor === undefined) {
